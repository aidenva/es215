...
def build_clusters(
    ...
    plt.tight_layout()
    plt.savefig(f"{tagged_prefix}_demand_grid.png", dpi=200)
    plt.close()
    ...
    with open(f"{tagged_prefix}_coord_shift.txt", "w") as f:
        f.write(f"Run ID: {run_id}\n")
        f.write("Graph coordinate transformation (origin at grid center):\n")
    ...
    clusters_csv = f"{tagged_prefix}_clusters.csv"
    ...
    grid_csv = f"{tagged_prefix}_grid_points.csv"
    ...
    fig.tight_layout()
    fig.savefig(f"{tagged_prefix}_cluster_centers.png", dpi=200)
    plt.close(fig)

    print("Saved cluster centers plot to:", f"{tagged_prefix}_cluster_centers.png")
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage:")
        print(
            "  python clusters_from_raster.py "
